...
@interface PublishBaseController ()<UICollectionViewDelegate,UICollectionViewDataSource,JJPhotoDelegate,ImagePickerSheetDelegate>{
    NSString *pushImageName;
    //添加图片提示
    UILabel *addImageStrLabel;
}
@property (nonatomic, strong) ImagePickerSheet *imgPickerActionSheet;
...
//    [_shareCollectionView registerClass:[PhotoCell class] forCellWithReuseIdentifier:@"cell"];
...
- (void) tapProfileImage:(UITapGestureRecognizer *)gestureRecognizer{
    ...
    if (index == (_imageArray.count)) {
        ...
        //点击放大查看
        PhotoCell *cell = (PhotoCell*)[_pickerCollectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
...
- (void)addNewImg{
    if (!_imgPickerActionSheet) {
        _imgPickerActionSheet = [[ImagePickerSheet alloc] init];
...
- (void)deletePhoto:(UIButton *)sender{
    ...
    for (NSInteger item = sender.tag; item <= _imageArray.count; item++) {
        PhotoCell *cell = (PhotoCell*)[self.pickerCollectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:item inSection:0]];
