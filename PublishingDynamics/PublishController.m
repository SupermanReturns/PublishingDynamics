...
@interface PublishController ()<UITextViewDelegate,UIScrollViewDelegate>{
    //备注文本View高度
...
- (void)viewDidLoad {
    ...
    backgroundView.frame=CGRectMake(0, 0, SCREENWIDTH, 64);
    [self.view addSubview:backgroundView];
    
    UILabel *tit=[[UILabel alloc]init];
    tit.frame=CGRectMake((SCREENWIDTH-230)*0.5, 0, 230, 44);
    tit.font=[UIFont systemFontOfSize:15];
    tit.textAlignment=NSTextAlignmentCenter;
    tit.text=@"分享新鲜事";
    [backgroundView addSubview:tit];
    
    UIButton *calBtn=[[UIButton alloc]init];
    calBtn.frame=CGRectMake(8,0 , 60, 44);
    [calBtn addTarget:self action:@selector(cancelClick:) forControlEvents:UIControlEventTouchUpInside];
    [calBtn setTitle:@"取消" forState:UIControlStateNormal];
    [backgroundView addSubview:calBtn];
